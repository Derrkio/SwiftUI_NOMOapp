...
actor AudioPreviewService {
    ...
    func previewURL(for track: PlayerTrack) async -> URL? {
    ...
    private func searchPreview(term: String) async -> URL? {
        ...
        do {
            ...
            return previewURLString.flatMap(URL.init(string:))
        } catch {
            return nil
        }
    }
...
class AppState: ObservableObject {
    ...
    @Published var playerMessage = "Select a track"
    ...
    func play(track: PlayerTrack) {
        currentTrack = track
        isLoadingTrack = true
        playerMessage = "Loading preview..."

        Task {
            let previewURL = await AudioPreviewService.shared.previewURL(for: track)
            await MainActor.run {
                ...
                guard let previewURL else {
                    ...
                    isPlaying = false
                    player.pause()
                    playerMessage = "No preview available for this track"
                    return
                }
                ...
                player.play()
                isPlaying = true
                playerMessage = "Now playing 30-second preview"
            }
        }
    }
    ...
    func togglePlayback() {
        guard currentPreviewURL != nil else { return }
    ...
    func autoplayIfNeeded() {
        guard !isPlaying, !isLoadingTrack, let currentTrack else { return }
                    ...
                    VStack(alignment: .leading, spacing: 20) {
                        HStack {
                            VStack(alignment: .leading) {
                                ...
                                Text("What did you miss?").font(.system(size: 14)).foregroundColor(Color.nomoGray)
                            }
                            Spacer()
                        ...
                        if let featured = Artist.featured, let concert = featured.concerts.first {
                            SectionLabel(text: "FEATURED").padding(.horizontal, 20)
                            FeaturedBanner(artist: featured, concert: concert)
                                .padding(.horizontal, 20)
                                .onTapGesture { selectedArtist = featured }
                        }

                        SectionLabel(text: "FOR YOU").padding(.horizontal, 20)
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: 14) {
                                ForEach(Artist.forYouPicks) { recommendation in
...
struct ArtistView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $selectedConcert) { concert in
            ConcertDetailView(artist: artist, concert: concert)
        }
    }
}
                            ...
                            ZStack(alignment: .bottomLeading) {
                                ...
                                VStack(alignment: .leading, spacing: 18) {
                                    HStack {
                                        ...
                                        Spacer()
                                        if let videoURL = currentTrack.clip.videoURL {
                                            Button {
                                                openURL(videoURL)
                                            } label: {
                            ...
                            VStack(alignment: .leading, spacing: 12) {
                                ...
                                HStack(spacing: 14) {
                                    PlayerActionButton(
                                        ...
                                        label: appState.isPlaying ? "Pause" : "Play",
                                        accent: .nomoViolet,
                                        filled: true
                                    ) {
...
struct PlayerLoadingView: View {
    ...
    var body: some View {
        ZStack {
            LinearGradient(
                colors: [Color.black, Color.nomoBG, Color.nomoPurple.opacity(0.85)],
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
            .ignoresSafeArea()
